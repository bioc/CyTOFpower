...
analyses: CytoGLMM [ref to add] and diffcyt [ref to add].
...
- the number of donors: the design of the data is paired, so the number of samples
will be calculated accordingly, twice the number of donors;
- the subject effect: it defines how different the donors are from each others;
- the number of cells per samples;
- the total number of markers tested in the experiment;
- the fold change \rho of each differentially expressed marker: the mean of these
markers will be multiplied by \rho in one condition;
- the distribution parameters for each marker: see details below.

The mean of each marker is drawn from a Gamma distribution: 
...
- mean $\mu_{0}$: with $\mu_{0} = k  \cdot \theta > 0$
- subject effect $S$: $S = \sqrt{k  \cdot \theta^{2}}$

The mean of the differentially expressed marker(s) is then multiplied by the 
fold change defined earlier: $\mu_{1,ij}= \mu_{0,ij} \cdot \rho_j$
...
data and the power is computated on request based on the parameters choosen by
the user.

### Precomputed dataset 

This panel allows the user to search a grid of parameters that have been pre-computed. 
